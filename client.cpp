...
void worker_thread_function (BoundedBuffer& request, BoundedBuffer& response, FIFORequestChannel* chan, int m) {
...
int main (int argc, char* argv[]) {
    ...
    vector<FIFORequestChannel*> vec_fifo;
    ...
    for(int i = 0; i < w; i++){
        FIFORequestChannel* temp = vec_fifo[i];
    ...
    for(int i = 0; i < w; i++){//quits all fifo
        vec_fifo[i]->cwrite((char *) &q, sizeof(MESSAGE_TYPE));
        delete vec_fifo[i];
    }
